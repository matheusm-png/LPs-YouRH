 ...
 * integração RD Station API de Conversões
 */
...
(function () {
  ...
  var validators = {
    nome: function (v) {
      var parts = v.trim().split(' ').filter(function(p){ return p.length > 0; });
      return v.trim().length >= 3 && parts.length >= 2 && parts[1].length >= 1;
    },
    email: function (v) {
  ...
  function hideLoading(btn) {
    btn.disabled = false;
    btn.innerHTML = btn.dataset.original || 'Quero uma demonstração gratuita!';
  }
  ...
  function sendToRdStation(data) {
    return fetch(RD_API_URL, {
      ...
      body: JSON.stringify({
        ...
        event_family: 'CDP',
        payload: {
          conversion_identifier: 'lp-youeduca',
          name:                data.nome,
          email:               data.email,
  ...
  function init() {
    ...
    form.addEventListener('submit', function (e) {
      ...
      sendToRdStation(data)
        ...
        .catch(function (err) {
          console.error('[YouEduca Form] Erro RD Station:', err);
