(function () {
  ...
  var FORM_ID        = 'lp-patinho-feio';
  var PAGE_NAME      = 'Patinho Feio do RH - YouRH';
  var PAGE_CATEGORY  = 'Landing Page - Marketing';
  ...
  function fireScrollDepth(pct) {
    var id = generateEventId();
    var base = basePayload('ViewContent', id);
    base.event = 'ViewContent'; base.content_name = PAGE_NAME; base.content_category = PAGE_CATEGORY;
    base.value = pct; base.currency = 'BRL'; base.scroll_percentage = pct;
    push(base);
  }
  ...
  function initScrollTracking() {
    ...
    function checkScroll() {
      ...
      [25, 50, 75, 100].forEach(function (mark) {
        if (!milestones[mark] && pct >= mark) { milestones[mark] = true; fireScrollDepth(mark); }
      });
      ticking = false;
    }
    window.addEventListener('scroll', function () { if (!ticking) { ticking = true; requestAnimationFrame(checkScroll); } }, { passive: true });
  }
