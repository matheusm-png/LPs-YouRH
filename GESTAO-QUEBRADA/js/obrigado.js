(function () {
  ...
  function fireLead() {
    ...
    var payload = {
      ...
      event_name:   'Lead',
      content_name: 'lp-gestao-quebrada',
      page_url:     window.location.href,
      timestamp:    Math.floor(Date.now() / 1000),
      user_data:    { email: hashes.email || null, phone_number: hashes.phone || null }
    };

    var fbp = getCookie('_fbp'); var fbc = getCookie('_fbc');
  ...
  if (document.readyState === 'loading') { document.addEventListener('DOMContentLoaded', fireLead); } else { fireLead(); }
})();
