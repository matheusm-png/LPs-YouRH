...
(function () {
  ...
  // ── INIT ─────────────────────────────────────────────────────────
  function init() {
    var form = document.getElementById('lp-forca-tripla-2');
    if (!form) return;

    populateHiddenUtmFields(form);
    populateHiddenMetaFields(form);

    var destination = form.getAttribute('action') || 'https://lp.yourh.com.br/forca-tripla-2/obrigado.html';
    var redirected  = false;

    // Netlify retorna 404 para POST em .html estático — bloqueamos o POST
    // nativo e fazemos o redirect por conta própria via GET.
    ...
    form.addEventListener('submit', function (e) {
      ...
      populateHiddenUtmFields(form);
      populateHiddenMetaFields(form);
      var tel = form.querySelector('[data-field="telefone"]');
